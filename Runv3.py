...
WIN = pygame.display.set_mode((WIDTH*2, HEIGHT*1.9))
pygame.display.set_caption("Gesture AI")
FPS = 50
Red = (255, 0, 0)
FONT = pygame.font.SysFont('avenir', 100)

model = torch.load(os.path.join('data/model'))
model.eval()
...
def main():
    ...
    while run:
        ...
        variable = pygame.transform.rotate(
            pygame.pixelcopy.make_surface(frame2), 270)
        variable2 = pygame.transform.scale(variable, (WIDTH, HEIGHT))

        pygame.draw.rect(WIN, (1, 11, 19), (0, 0, WIDTH*2, HEIGHT*2))

        WIN.blit(variable2, (WIDTH/2, 0))

        draw_text = FONT.render(f'{output}', 1, (0, 255, 0))
        WIN.blit(draw_text, (WIDTH - draw_text.get_width(),
